...
parser = argparse.ArgumentParser(
    ...
    usage="To perform any task listed below, you will need to create or use a new/existing session.\nEach "
          "new/existing session can be accessed by its name provided using the --profile flag.\n\nYou don't need to "
          "provide API_ID, API_HASH, phone number or bot token etc to perform any task again & again!\nOnce you "
          "successfully login & created a session then you just need to provide its name using --profile flag and you "
          "will be logged in without any authentication flow (until you terminate the session using Telegram).",
...
parser.add_argument("--export_string", action="store_true",
                    help="Generate & display session string using existing session file.")
...
parser.add_argument("--chat_id",
                    help="Identity of chat to send the file to? can be username, phone number (international format) "
                         "or ID number. By default to Saved Messages.")
    ...
    if args.as_photo:
        if Path(args.path).is_file():
            try:
                ...
                client.send_photo(chat_id, args.path, caption=args.caption, progress=upload_progress,
                                  disable_notification=args.silent, has_spoiler=args.spoiler)
            ...
            for _path in Path(args.path).glob("**/*") if args.recursive else Path(args.path).glob("*"):
                ...
                if Path(_path).is_file():
                    try:
                        client.send_photo(chat_id, _path, caption=args.caption, progress=upload_progress,
                                          disable_notification=args.silent, has_spoiler=args.spoiler)
        ...
        if Path(args.path).is_file():
            try:
                ...
                client.send_video(chat_id, args.path, progress=upload_progress, caption=args.caption,
                                  has_spoiler=args.spoiler, width=args.width, height=args.height, thumb=args.thumb,
                                  file_name=filename, supports_streaming=args.disable_stream,
            ...
            for _path in Path(args.path).glob("**/*") if args.recursive else Path(args.path).glob("*"):
                if Path(_path).is_file():
                    try:
                        ...
                        if args.prefix:
                            filename = args.prefix + filename
                        client.send_video(chat_id, _path, progress=upload_progress, caption=args.caption,
                                          has_spoiler=args.spoiler, width=args.width, height=args.height,
                                          thumb=args.thumb, file_name=filename, supports_streaming=args.disable_stream,
                                          disable_notification=args.silent)
                        Path(_path).unlink(missing_ok=True) if args.delete_on_done else None
                    except Exception as error_code:
                        print(f"An error occurred!\n{error_code}")
        ...
        if Path(args.path).is_file():
            try:
                ...
                if args.prefix:
                    filename = args.prefix + filename
                client.send_audio(chat_id, args.path, progress=upload_progress, caption=args.caption,
                                  performer=args.artist, title=args.title, thumb=args.thumb, file_name=filename,
                                  disable_notification=args.silent)
                Path(args.path).unlink(missing_ok=True) if args.delete_on_done else None
            except Exception as error_code:
                print(f"An error occurred!\n{error_code}")
                ...
                if Path(_path).is_file():
                    try:
                        ...
                        if args.prefix:
                            filename = args.prefix + filename
                        client.send_video(chat_id, _path, progress=upload_progress, caption=args.caption,
                                          performer=args.artist, thumb=args.thumb, file_name=filename,
                                          disable_notification=args.silent)
                        Path(_path).unlink(missing_ok=True) if args.delete_on_done else None
                    except Exception as error_code:
                        print(f"An error occurred!\n{error_code}")
        ...
        if Path(args.path).is_file():
            try:
                ...
                if args.prefix:
                    filename = args.prefix + filename
                client.send_voice(chat_id, args.path, progress=upload_progress, caption=args.caption,
                                  disable_notification=args.silent)
                Path(args.path).unlink(missing_ok=True) if args.delete_on_done else None
            except Exception as error_code:
                print(f"An error occurred!\n{error_code}")
            ...
            for _path in Path(args.path).glob('**/*') if args.recursive else Path(args.path).glob("*"):
                if Path(_path).is_file():
                    try:
                        ...
                        if args.prefix:
                            filename = args.prefix + filename
                        with open(_path, 'rb') as video_file:
                            client.send_video(chat_id, video_file, progress=upload_progress, caption=args.caption,
                                              disable_notification=args.silent)
                        Path(_path).unlink(missing_ok=True) if args.delete_on_done else None
                    except Exception as error_code:
                        print(f"An error occurred!\n{error_code}")
        ...
        if Path(args.path).is_file():
            try:
                ...
                if args.prefix:
                    filename = args.prefix + filename
                client.send_video_note(chat_id, args.path, progress=upload_progress, caption=args.caption,
                                       disable_notification=args.silent)
                Path(args.path).unlink(missing_ok=True) if args.delete_on_done else None
            except Exception as error_code:
                print(f"An error occurred!\n{error_code}")
            ...
            for _path in Path(args.path).glob("**/*") if args.recursive else Path(args.path).glob("*"):
                if Path(_path).is_file():
                    try:
                        ...
                        if args.prefix:
                            filename = args.prefix + filename
                        client.send_video_note(chat_id, _path, progress=upload_progress, caption=args.caption,
                                               disable_notification=args.silent)
                        Path(_path).unlink(missing_ok=True) if args.delete_on_done else None
                    except Exception as error_code:
                        print(f"An error occurred!\n{error_code}")
        ...
        try:
            ...
            if args.prefix:
                filename = args.prefix + filename
            client.send_document(chat_id, args.path, progress=upload_progress, caption=args.caption,
                                 force_document=True, file_name=filename, thumb=args.thumb,
                                 disable_notification=args.silent)
            Path(args.path).unlink(missing_ok=True) if args.delete_on_done else None
        except Exception as error_code:
            print(f"An error occurred!\n{error_code}")
        ...
        for _path in Path(args.path).glob("**/*") if args.recursive else Path(args.path).glob("*"):
            if Path(_path).is_file():
                try:
                    ...
                    if args.prefix:
                        filename = args.prefix + filename
                    with open(_path, 'rb') as document_file:
                        client.send_document(chat_id, document_file, progress=upload_progress, caption=args.caption,
                                             force_document=True, file_name=filename, thumb=args.thumb,
                                             disable_notification=args.silent)
                    Path(_path).unlink(missing_ok=True) if args.delete_on_done else None
                except Exception as error_code:
